...
def __main__():
    ...
    def plot_bessel_heat_distribution(a0, a1, P, eta, r_g, rr_1, r_t1, rr_2, r_t2, A, C1, C2, factor, i ):
        rg, rr1, rt1, rr2, rt2, = r_g*1.0e-6, rr_1*1.0e-6, r_t1*1.0e-6, rr_2*1.0e-6, r_t2*1.0e-6,  # scaling unit to meter
        A = A*1e7/1e6
        ...
        r = (x**2 + y**2)**0.5
        F = np.where(rr2 + rt2 - r < 0, 0, 1) * factor
        Y1 = np.exp(-2*rr1**2/(rt1**2)) + (2*rr1/rt1)*(np.pi/2)**0.5 * math.erfc(-2**0.5*rr1/(rt1))
        Y2 = np.exp(-2*rr2**2/(rt2**2)) + (2*rr2/rt2)*(np.pi/2)**0.5 * math.erfc(-2**0.5*rr2/(rt2))
     
        Q0 = F*((C1*eta*A*a0*P)/(np.pi*rg**2))*(np.exp(-C2*(r**2/rg**2)))
        Q1 = F*((C1*eta*A*a1*P)/(np.pi*rt1**2*Y1)) * (np.exp(-C2*((r-rr1)**2/(rt1**2))))
        Q2 = F*((C1*eta*A*a2*P)/(np.pi*rt2**2*Y2)) * (np.exp(-C2*((r-rr2)**2/(rt2**2))))
        Q = Q0 + Q1 + Q2
    ...
    eta = cm1.slider(r'Efficiency $[\eta]$',                                min_value=0.0,  max_value=1.0,      value=0.8,      step=0.0001)
    rG = cm1.slider(r'''Gaussian Beam Radius $$(r_G$$ $$\mu m)$$''',        min_value=1.0,  max_value=75.0,     value=35.0,     step=0.1)
    r1 = cm1.slider(r'''Ring 1 radius $$(Rr_1$$ $$\mu m)$$''',              min_value=1.0,  max_value=100.0,    value=40.0,     step=0.1)
    rT1 = cm1.slider(r'''Ring 1 half thickness $$(r_{t1}$$ $$\mu m)$$''',   min_value=1.0,  max_value=50.0,     value=10.0,      step=0.01)
    r2 = cm1.slider(r'''Ring 2 radius $$(Rr_2$$ $$\mu m)$$''',              min_value=1.0,  max_value=100.0,    value=50.0,     step=0.1)
    rT2 = cm1.slider(r'''Ring 2 half thickness $$(r_{t2}$$ $$\mu m)$$''',   min_value=1.0,  max_value=25.0,     value=5.0,      step=0.01)
    

    a0 = cm1.slider(r'P$_{G} \, proportion \, \, (a_0)$',    min_value=0.0, max_value=1.0, value=0.69, step=0.01)
    a1 = cm1.slider(r'P$_{R1} \, proportion \, \, (a_1)$', min_value=0.0, max_value=a0,  value=0.23, step=0.01)
    
    A = cm1.slider(r'''Absorptivity $$(A \times 10^7/m)$$''', min_value=0.00001, max_value=20.0, value=8.50, step=0.1)
    C1 = cm1.slider(r'''Constant $$(C_1)$$''', min_value=0.0000001, max_value=4.0, value=1.595769122, step=0.1)
    C2 = cm1.slider(r'''Constant $$(C_2)$$''', min_value=0.0000001, max_value=4.0, value=2.0, step=0.1)
    i = cm1.slider('Cmap  ', min_value=0, max_value=9, value=7, step=1)
    factor = 1.0e-4

    fig_Bessel = plot_bessel_heat_distribution(a0, a1, P, eta, rG, r1, rT1, r2, rT2, A, C1, C2, factor, i, )
